...
public:
    ...
    int time_update_;
    std::string mq_path_;
    std::string informerSqlStr;

    float range_short_term_ = 0.5;
    float range_long_term_ = 0.5;
    float range_context_ = 0.5;
    float range_search_ = 0.5;
    float range_category_ = 0.5;
    int retargeting_percentage_ = 50;
    ...
    bool toLog()
    {
        return logCoreTime || logIP || logCountry || logRegion || logCookie
        || logContext || logSearch || logInformerId || logLocation;
    }
